...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label for="{{$name}}" class="col-form-label">{{ \App\Model\Category\Helper\AdminHelper::getFormLabel($name) }}</label>
            <textarea id="{{$name}}" class="form-control{{ $errors->has($name) ? ' is-invalid' : '' }}" name="{{$name}}" rows="5" required>{{ old($name, $category->description_ru) }}</textarea>
            @if ($errors->has($name))
                <span class="invalid-feedback"><strong>{{ $errors->first($name) }}</strong></span>
            @endif
        </div>
        ...
        <div class="form-group">
            <label for="{{$name}}" class="col-form-label">{{ \App\Model\Category\Helper\AdminHelper::getFormLabel($name) }}</label>
            <textarea id="{{$name}}" class="form-control{{ $errors->has($name) ? ' is-invalid' : '' }}" name="{{$name}}" rows="5">{{ old($name, $category->description_uk) }}</textarea>
            @if ($errors->has($name))
                <span class="invalid-feedback"><strong>{{ $errors->first($name) }}</strong></span>
            @endif
        </div>
